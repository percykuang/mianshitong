...
- 这次调整同时明确限制了技术问答排版边界：不要使用 H1、不要使用 emoji 标题、不要输出 mermaid 图和 Markdown 分割线；重点是提升结构感，而不是做花哨排版。
...
- 为了让 Web 端普通聊天更像“前端 AI 面试官产品”，当前已显式增加一层通用回复策略，而不再只依赖模型自由生成：服务端普通聊天现支持高频入口模板短路（简历优化、问候语、简单算术回拉、自我介绍、项目亮点提炼），并通过统一 system prompt 强化领域角色、缺信息先索取、少用花哨 Markdown 和禁止横线分割。
