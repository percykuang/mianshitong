...
export function KnowledgeDocumentCreateView() {
  ...
  useEffect(() => {
    form.setFieldsValue({
      category: KNOWLEDGE_DOCUMENT_CATEGORY_OPTIONS[0]?.value,
      contentShape: KNOWLEDGE_DOCUMENT_CONTENT_SHAPE_OPTIONS[0]?.value,
      tags: [],
      isPublished: false,
    });
  }, [form]);

  const buildPayload = async (): Promise<KnowledgeDocumentPayload | null> => {
  ...
  const persistDocument = async (mode: 'exit' | 'continue') => {
    if (saving) {
      return;
    }
    ...
    setSaving(true);
    try {
      const response = await fetch('/api/knowledge-documents/items', {
        method: 'POST',
      ...
      if (mode === 'continue') {
        const sticky = {
          ...
          contentShape: payload.contentShape,
          tags: payload.tags,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      >
        <Space>
          <Button onClick={() => void persistDocument('continue')} loading={saving}>
            保存并继续添加
          </Button>
          <Button type="primary" onClick={() => void persistDocument('exit')} loading={saving}>
            保存
          </Button>
        </Space>
      </footer>
