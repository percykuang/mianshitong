...
import { createDraftChatSession, toSessionSummary } from '../lib/chat-session-draft';
...
function createDeps(overrides: Partial<Parameters<typeof useChatDeleteActions>[0]> = {}) {
  return {
    ...
    fetchSessionById: vi.fn(async (sessionId: string) =>
      createDraftChatSession('deepseek-chat', sessionId),
    ),
...
describe('useChatDeleteActions', () => {
  it('删除的不是当前激活会话时不会切换当前会话', async () => {
    const nextSession = createDraftChatSession('deepseek-chat', 'next_session');
  ...
  it('删除当前会话且下一个会话已缓存时会直接切换缓存', async () => {
    const cachedNext = createDraftChatSession('deepseek-chat', 'cached_next_session');
    const deps = createDeps({
      activeSessionId: 'active_session',
  ...
  it('删除当前会话且下一个会话未缓存时会远端拉取', async () => {
    const remoteNext = createDraftChatSession('deepseek-chat', 'remote_next_session');
    const deps = createDeps({
      activeSessionId: 'active_session',
